...
- **PV sell strategy** — after the main greedy pass, `_plan_pv_sell_slots()` plans which solar slots should export to the grid via the Emaldo third-party PV switch (`switch.power_store_third_party_pv`) instead of charging the battery. Two pathways: **excess-solar** (remaining solar ≥ needed-to-fill → any sell price > wear_cost is pure profit) and **opportunity-value** (sell > future_max_buy × round_trip − wear_cost). A recharge-feasibility check ensures no planned discharge slot drops below `soc_min`. Enabled via `switch.battery_optimizer_pv_strategy`. After sell slots are planned, `_correct_soc_for_pv_sells()` recomputes the SoC trajectory so the dashboard forecast is accurate. Gap-fill post-processing closes gaps of ≤3 consecutive non-sell slots between sell windows to prevent rapid relay toggling.
...
7. **Plan PV sell slots** (when `enable_pv_strategy=True`) — `_plan_pv_sell_slots()` runs a second greedy pass over solar slots, marking each as sell-to-grid or charge-battery. Gap-fill post-processing closes short gaps between sell windows. Then `_correct_soc_for_pv_sells()` does a forward SoC correction pass. Both today and tomorrow plans go through this step.
