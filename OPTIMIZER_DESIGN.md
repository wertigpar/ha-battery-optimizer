...
- **PV sell strategy** — after the main greedy pass, `_plan_pv_sell_slots()` selects a single **cutover slot T** (≤ noon) and sells all solar before T to the grid (PV switch OFF), then charges the battery uninterrupted from T onward. T is the latest slot where remaining solar after T can still fill the battery to `soc_max`. If total solar is insufficient to fill the battery (< 95 % of needed), selling is skipped entirely. **Charge-to-floor-first**: when the battery enters the window below `soc_min + buffer` (e.g. after a cloudy night), the first solar charges the battery back to the floor before any selling starts; if the floor cannot be reached before the cutover, selling is skipped for the day. Grid-charge slots are never overridden. Enabled via `switch.battery_optimizer_pv_strategy`. After sell slots are planned, `_correct_soc_for_pv_sells()` recomputes the SoC trajectory so the dashboard forecast is accurate.
