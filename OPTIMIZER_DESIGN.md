...
- **Plan today + tomorrow** — when next-day prices are available (after the Nord Pool day-ahead publish), the optimizer also plans all 96 slots for tomorrow. Both plans are pushed via a rolling 96-slot E2E window: positions `[now_slot..95]` carry today's plan, positions `[0..now_slot-1]` carry tomorrow's plan.
...
3. **Find Case A discharge candidates** — existing stored energy can be discharged for self-consumption when `buy_price > wear_cost` AND `total_discharge_budget > 0`. Discharge candidates are grid slots only (sorted by buy price descending).
   ...
   **NIGHT RESERVATION (COMBINED days, bias-based)**: when solar cannot fully refill the battery, the single combined pool covers evening → morning → night LAST, so the cheapest pre-solar night grid buys starve even though the battery holds stored energy. Before the combined pass, a slice of the initial pool is reserved for those slots: `night_reserve_kwh = initial_usable_kwh × min(1, bias × refill_fraction / 0.95)`, where `refill_fraction = (peak_soc_from_min − soc_min) / (soc_max − soc_min)` scales the reservation by how much solar refill actually happened (bias 1.0 + full refill reserves the whole initial pool). Reserved slots are the cheapest night slots first (ascending buy, latest slot on ties — `sorted((s for s in profitable_discharge if s < first_solar_slot), key=(round(buy,4), −s))`), assigned per-slot draw `load/η_d` until the reserve is exhausted. The combined loop skips reserved slots and allocates the remaining budget buy-desc as before — **the reserve is not subtracted from the combined budget** (no double-charge: the correction below is the floor safety net and trims the cheapest committed slot — the reserved tail first — when a true trajectory violates the floor). Bias is a single module constant `_NIGHT_RESERVE_BIAS` (default **0.6**, chosen safe by A/B: 0.6 → 2.8337 €/day, 0.8 → 2.9819, plateau 2.9822, min SoC 27.7 % ≥ 20 % floor; bias 0 → byte-identical legacy). Raise/lower the constant to tune; bias ≥ 1.0 reserves the whole initial pool for night priority.
