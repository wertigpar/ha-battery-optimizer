...
| **Rest of Day Baseline Cost** | monetary | Estimated cost for rest of today with **no battery** (€). Solar self-consumes first: only the net deficit is imported and only the net surplus exported — the same pricing convention as the internal plan and the optimizer plan, so the three are directly comparable | `import_cost`, `export_revenue`, `import_kwh`, `export_kwh`, `import_energy`, `import_transfer`, `import_tax`, `import_commission`, `export_energy`, `export_commission`, `remaining_slots` |
...
| **Tomorrow Baseline Cost** | monetary | Estimated cost for tomorrow with **no battery** (€). Solar self-consumes first: only the net deficit is imported and only the net surplus exported — the same pricing convention as the internal plan and the optimizer plan, so the three are directly comparable | `import_cost`, `export_revenue`, `import_kwh`, `export_kwh`, `import_energy`, `import_transfer`, `import_tax`, `import_commission`, `export_energy`, `export_commission`, `remaining_slots` |
